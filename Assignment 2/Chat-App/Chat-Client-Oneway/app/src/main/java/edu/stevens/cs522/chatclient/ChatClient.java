...
public class ChatClient extends Activity implements OnClickListener {
	...
	public void onClick(View v) {
		try {
			...
			String des_host=destinationHost.getText().toString();
			InetAddress destAdd= InetAddress.getByName(des_host);


			String host_port=destinationPort.getText().toString();
			int destPort = Integer.parseInt(host_port);


			String des_message=messageText.getText().toString();
			byte[] sendData= des_message.getBytes(); // Combine sender and message text; default encoding is UTF-8


			DatagramPacket sendPacket = new DatagramPacket(sendData, sendData.length, destAdd, destPort);
			Log.e("send",sendPacket.toString());
